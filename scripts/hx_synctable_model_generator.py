...
class SyncTableModelGenerator:
    ...
    def __init__(self, output_dir: str = ".", hx_model_file: str = "../param/dxl_model/hx5_d20_rl.model"):
        ...
        self.finger_unit_info = {
            "Present Current": {"value": "1.0", "unit": "raw", "sign_type": "signed", "offset": "0.0"},
            "Present Velocity": {"value": "0.0239691227", "unit": "rad/s", "sign_type": "signed", "offset": "0.0"},
            "Present Position": {"value": "0.0015339807878856412", "unit": "rad", "sign_type": "signed", "offset": "-3.14159265359"},
            "Goal Current": {"value": "1.0", "unit": "raw", "sign_type": "signed", "offset": "0.0"},
            "Goal Position": {"value": "0.0015339807878856412", "unit": "rad", "sign_type": "signed", "offset": "-3.14159265359"}
        }

        self.pressure_unit_info = {
            f"Present Pressure {i}": {"value": "1.0", "unit": "raw", "sign_type": "unsigned", "offset": "0.0"}
    ...
    def parse_hx_model_file(self) -> None:
        ...
        try:
            ...
            print(f"Parsed {len(self.synctable_base_addresses)} SyncTables from {self.hx_model_file}")
            for synctable_num, addresses in self.synctable_base_addresses.items():
                print(f"  SyncTable {synctable_num}: read_base={addresses.get('read_base', 'N/A')}, write_base={addresses.get('write_base', 'N/A')}")
    ...
    def calculate_synctable_addresses(self, synctable_num: int,
                                    finger_joints: List[int],
                                    pressure_sensors: List[int]) -> Tuple[List[int], List[int]]:
        """
        ...
        for sensor_num in pressure_sensors:
            ...
            sensor_read_addrs = list(range(current_read_addr, current_read_addr + self.pressure_read_size))
    ...
    def generate_finger_joint_model(self, joint_number: int,
        ...
        for data_name, info in self.finger_unit_info.items():
            unit_info_lines.append(f"{data_name}	{info['value']}	{info['unit']}	{info['sign_type']}	{info['offset']}")
    ...
    def generate_synctable_configuration(self, synctable_groups: Dict[int, Dict]) -> None:
        ...
        for synctable_num, config in synctable_groups.items():
            ...
            print(f"Processing SyncTable {synctable_num}: joints {finger_joints}, sensors {pressure_sensors}")
            print(f"  Will generate files: hx5_d20_r_synctable_{synctable_num}_device_1.model to hx5_d20_r_synctable_{synctable_num}_device_{len(finger_joints) + len(pressure_sensors)}.model")
            ...
            for joint_num in finger_joints:
                ...
                content = self.generate_finger_joint_model(joint_num, joint_read_addrs, joint_write_addrs)
            ...
            for sensor_num in pressure_sensors:
                sensor_read_addrs = read_addresses[current_read_idx:current_read_idx + self.pressure_read_size]
    ...
    def generate_simple_hand_models(self, num_synctables: int = 5, joints_per_synctable: int = 4) -> None:
        """
        ...
        for synctable_num, config in synctable_groups.items():
            print(f"  SyncTable {synctable_num}: joints {config['finger_joints']}, sensor {config['pressure_sensors']}")
    ...
    def analyze_existing_models(self, model_dir: str = "../param/dxl_model") -> Dict:
        ...
        return {
            "message": "Analysis functionality would extract SyncTable configuration from existing models"
        }
...
def main():
    ...
    generator = SyncTableModelGenerator(output_dir="../param/dxl_model", hx_model_file="../param/dxl_model/hx5_d20_rl.model")
    ...
    # # Example 2: Generate custom models
    # print("\n2. Generating custom models...")
    # custom_config = {
    #     "synctable_groups": {
    #         1: {"finger_joints": [1, 2, 3, 4], "pressure_sensors": [1]},
    #         2: {"finger_joints": [5, 6, 7, 8], "pressure_sensors": [2]},
    ...
    #     "pressure_write_size": 0
    # }
    # generator.generate_custom_hand_models(custom_config)
